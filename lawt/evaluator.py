...
class Evaluator(object):
    ...
    def enc_dec_step(self, data_type, task, scores):
        ...
        for (x1, len1), (x2, len2), nb_ops in iterator:
            ...
            for i in range(bs):
                ...
                # sanity check
                assert gen["src"] == src and gen["tgt"] == tgt and gen["i"] == i
        ...
        logger.info(f"{_n_valid}/{_n_total} ({100.0 * _n_valid / _n_total}%) equations were evaluated correctly.")
        ...
        scores[f"{data_type}_{task}_acc_d1"] = 100.0 * (n_perfect_match + n_valid_d1) / _n_total
        scores[f"{data_type}_{task}_acc_d2"] = 100.0 * (n_perfect_match + n_valid_d2) / _n_total
        scores[f"{data_type}_{task}_acc_nb"] = 100.0 * (n_perfect_match + n_valid_nb) / _n_total
        ...
        for i in range(len(n_total)):
            ...
            scores[f"{data_type}_{task}_acc_{e}"] = 100.0 * n_valid[i].item() / max(n_total[i].item(), 1)
            if n_valid[i].item() > 0:
                logger.info(
                    f"{e}: {n_valid[i].item()} / {n_total[i].item()} "
                    f"({100.0 * n_valid[i].item() / max(n_total[i].item(), 1)}%)"
                )
    ...
    def enc_dec_step_beam(self, data_type, task, scores, size=None):
        ...
        scores[f"{data_type}_{task}_perfect"] = 100.0 * n_perfect_match / _n_total
        scores[f"{data_type}_{task}_correct"] = 100.0 * (n_perfect_match + n_correct) / _n_total
        scores[f"{data_type}_{task}_beam_acc_d1"] = 100.0 * (n_perfect_match + n_valid_d1) / _n_total
        scores[f"{data_type}_{task}_beam_acc_d2"] = 100.0 * (n_perfect_match + n_valid_d2) / _n_total
        scores[f"{data_type}_{task}_beam_acc_nb"] = 100.0 * (n_perfect_match + n_valid_nb) / _n_total
        ...
        for i in range(len(n_total)):
            ...
            scores[f"{data_type}_{task}_beam_acc_{e}"] = 100.0 * n_valid[i].item() / max(n_total[i].item(), 1)

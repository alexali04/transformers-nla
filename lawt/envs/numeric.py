...
import lawt.envs.encoders as encoders
import lawt.envs.generators as generators
from lawt.dataset import EnvDataset

from ..utils import bool_flag

SPECIAL_WORDS = (
    ["<s>", "</s>", "<pad>", "(", ")"] + [f"<SPECIAL_{i}>" for i in range(10)] + ["T", "A", "D", "M", "E", "F", "I"]
)
logger = getLogger()
...
class NumericEnvironment(object):
    ...
    def __init__(self, params):
        ...
        self.additional_tolerance = [float(x) for x in params.more_tolerance.split(",") if len(x) > 0]
        ...
        assert self.input_encoder.limit <= 0.0 or params.max_input_coeff <= self.input_encoder.limit
        ...
        self.words = (
            SPECIAL_WORDS
            + self.common_symbols
            + sorted(list(set(self.output_encoder.symbols + self.input_encoder.symbols)))
        )
    ...
    def create_test_iterator(self, data_type, task, data_path, batch_size, params, size):
        """
        Create a dataset for this environment.
        """
    ...
    def register_args(parser):
        ...
        """
        parser.add_argument("--operation", type=str, default="invert_matrix", help="Operation performed")
        parser.add_argument("--cotraining_tasks", type=str, default="TADMEFI", help="Cotraining operations")

        parser.add_argument("--generator", type=str, default="uniform", help="Random generation of coefficients")
        parser.add_argument("--max_input_coeff", type=int, default=10, help="max coeff of input matrix")
        parser.add_argument(
            "--min_input_coeff", type=int, default=-1, help="min coeff of input matrix, if -1 min = max"
        )

        # generation parameters
        parser.add_argument("--force_dim", type=bool_flag, default=False, help="unique dimension in dataset")
        parser.add_argument("--first_dimension", type=int, default=5, help="first dimension if force_dim")
        parser.add_argument("--second_dimension", type=int, default=5, help="second dimension if force_dim")

        parser.add_argument("--min_dimension", type=int, default=5, help="minimum dimension of space")
        parser.add_argument("--max_dimension", type=int, default=5, help="maximum dimension of space")
        parser.add_argument("--rectangular", type=bool_flag, default=False, help="rectangular matrices allowed")

        parser.add_argument(
            "--eigen_distribution",
            type=str,
            default="semicircle",
            help="eigenvalue distribution (semicircle, positive, uniform, gaussian, laplace)",
        )
        parser.add_argument(
            "--eigen_test_distribution",
            type=str,
            default="semicircle",
            help="eigenvalue test distribution (semicircle, positive, uniform, gaussian, laplace)",
        )
        parser.add_argument(
            "--additional_test_distributions",
            type=str,
            default="",
            help="additional test sets, sets separated by ; generators by ,",
        )
        parser.add_argument("--noisy_input", type=bool_flag, default=False, help="add gaussian noise to input")
        ...
        parser.add_argument("--classic_eval", type=bool_flag, default=False, help="evaluate inversion as other ops")
        ...
        parser.add_argument("--output_encoding", type=str, default="float,2,1000", help="Encoder for output sequence")
        parser.add_argument("--input_encoding", type=str, default="float,2,1000", help="Encoder for input sequence")
        ...
        parser.add_argument("--more_tolerance", type=str, default="", help="additional tolerance limits")
        parser.add_argument("--eval_norm", type=str, default="d1", help="norm to use for evaluation, max, d1 or d2")

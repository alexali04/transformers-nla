...
class EnvDataset(Dataset):
    ...
    def load_chunk(self):
        ...
        logger.info(f"Loading data from {self.path} ... seekpos {self.seekpos}, basepos {self.basepos}")
        endfile = False
        ...
        logger.info(f"Loaded {len(self.data)} equations from the disk. seekpos {self.seekpos}, nextpos {self.nextpos}")
        if len(self.data) == 0:
            self.load_chunk()
